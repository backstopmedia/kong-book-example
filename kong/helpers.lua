...
local function modifier_response(state, arguments, level)
  ...
  assert(
    type(res) == "table" and type(res.read_body) == "function",
...
local function modifier_request(state, arguments, level)
  local generic =
    "The assertion 'request' modifier takes a http response" ..
    " object as input to decode the json-body returned by" .. " mock_upstream, to retrieve the proxied request."
  ...
  assert(
    request,
    "Expected the http response object to have a json encoded body," ..
      " but decoding gave error '" .. tostring(err) .. "'. Obtained body: " .. body .. "\n." .. generic
  )
...
luassert:register("assertion", "fail", fail, "assertion.fail.negative", "assertion.fail.negative")
...
luassert:register("assertion", "contains", contains, "assertion.contains.negative", "assertion.contains.positive")
...
luassert:register("assertion", "status", res_status, "assertion.res_status.negative", "assertion.res_status.positive")
luassert:register(
  "assertion",
  "res_status",
  res_status,
...
local function jsonbody(state, args)
  assert(
    args[1] == nil and rawget(state, "kong_request") or rawget(state, "kong_response"),
  ...
  if rawget(state, "kong_response") then
    ...
    if r.post_data and (r.post_data.kind == "json" or r.post_data.kind == "json (error)") and r.post_data.params then
...
luassert:register("assertion", "jsonbody", jsonbody, "assertion.jsonbody.negative", "assertion.jsonbody.positive")
...
local function res_header(state, args)
  ...
  assert(
    type(res) == "table" and type(res.headers) == "table",
...
luassert:register("assertion", "header", res_header, "assertion.res_header.negative", "assertion.res_header.positive")
...
local function req_form_param(state, args)
  ...
  local value
  if req.post_data and (req.post_data.kind == "form" or req.post_data.kind == "multipart-form") then
...
return {
  ...
  mock_upstream_url = MOCK_UPSTREAM_PROTOCOL .. "://" .. MOCK_UPSTREAM_HOST .. ":" .. MOCK_UPSTREAM_PORT,
  mock_upstream_ssl_protocol = MOCK_UPSTREAM_SSL_PROTOCOL,
  mock_upstream_ssl_port = MOCK_UPSTREAM_SSL_PORT,
  mock_upstream_ssl_url = MOCK_UPSTREAM_SSL_PROTOCOL .. "://" .. MOCK_UPSTREAM_HOST .. ":" .. MOCK_UPSTREAM_SSL_PORT,
